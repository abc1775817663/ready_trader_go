...
#                self.asks.add(self.ask_id)
  ...
  def on_order_book_update_message(self, instrument: int, sequence_number: int, ask_prices: List[int],
      ...
      if self.ETF_mid != 0 and self.FUTURE_mid !=0 and self.ETF_mid < self.FUTURE_mid and self.FUTURE_mid - self.ETF_mid - 0.002 * self.ETF_mid > 0 and self.position < POSITION_LIMIT:
       #    if self.position > 0:
       #        self.ask_id = next(self.order_ids)
       ...
       #        self.bid_id = next(self.order_ids)
       #        self.send_insert_order(self.bid_id, Side.BUY, bid_prices[0], -self.position, Lifespan.FILL_AND_KILL)
       #        self.asks.add(self.ask_id)
           if self.position < POSITION_LIMIT - 11:
               self.bid_id = next(self.order_ids)
               self.send_insert_order(self.bid_id, Side.BUY, bid_prices[0], LOT_SIZE, Lifespan.IMMEDIATE_OR_CANCEL)
               self.bids.add(self.bid_id)
      elif self.ETF_mid !=0 and self.FUTURE_mid !=0 and self.ETF_mid > self.FUTURE_mid and self.ETF_mid - self.FUTURE_mid - 0.002 * self.ETF_mid > 0:
       #    if self.position > 0:
       #        self.bid_id = next(self.order_ids)
       ...
       #        self.asks.add(self.ask_id)
           if self.position > -POSITION_LIMIT + 11:
               self.ask_id = next(self.order_ids)
               self.send_insert_order(self.ask_id, Side.SELL, ask_prices[0], LOT_SIZE, Lifespan.IMMEDIATE_OR_CANCEL)
               self.asks.add(self.ask_id)
